...
export async function GET(req: Request) {
    ...
    try {
        ...
        let query = supabase.from('saved_reflections').select('*').eq('user_id', sec.user?.id).order('created_at', { ascending: false });
        ...
        console.error('Reflection Fetch All API Error:', error.message);

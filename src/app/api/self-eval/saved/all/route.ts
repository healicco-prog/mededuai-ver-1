...
export async function GET(req: Request) {
    ...
    try {
        ...
        let query = supabase.from('saved_self_evals').select('*').eq('user_id', sec.user?.id).order('created_at', { ascending: false });
        ...
        console.error('Self-Eval Fetch All API Error:', error.message);

            ...
            <div className={`relative bg-gradient-to-br ${gradient} rounded-3xl p-8 md:p-10 min-h-[480px] flex flex-col shadow-2xl overflow-hidden`}>
                ...
                <div className="relative z-10 mt-6 flex items-center justify-between">
                    ...
                    <div className="flex gap-1">
                        {slides.map((_, i) => (
                            <button key={i} onClick={() => setCurrentSlide(i)} className={`h-2 rounded-full transition-all duration-300 ${i === currentSlide ? 'bg-white w-6' : 'bg-white/30 w-2 hover:bg-white/50'}`} />
                        ))}
                    </div>
                </div>
            </div>
            <div className="flex items-center justify-center gap-4">
                <button onClick={() => setCurrentSlide(prev => Math.max(0, prev - 1))} disabled={currentSlide === 0} className="px-6 py-3 rounded-xl bg-slate-900 text-white font-bold text-sm disabled:opacity-30 hover:bg-slate-800 transition-all flex items-center gap-2 shadow-lg">
                    <ChevronLeft className="w-4 h-4" /> Previous
                </button>
                <span className="text-sm font-black text-slate-400 px-4 tabular-nums">{currentSlide + 1} / {slides.length}</span>
                <button onClick={() => setCurrentSlide(prev => Math.min(slides.length - 1, prev + 1))} disabled={currentSlide === slides.length - 1} className="px-6 py-3 rounded-xl bg-slate-900 text-white font-bold text-sm disabled:opacity-30 hover:bg-slate-800 transition-all flex items-center gap-2 shadow-lg">
...
export default function TeacherLMSNotes() {
    ...
    // Build merged course list: staticCoursesList is the BASE (always fresh, never stale),
    // DB content is OVERLAID.
    ...
    // Start empty; synced to DB course IDs after hierarchy loads
    ...
    useEffect(() => {
        const fetchUser = async () => {
            ...
            if (session?.user) {
                ...
                const name = session.user.user_metadata?.full_name || session.user.user_metadata?.name || 'Teacher';
                setUserName(name);
    ...
    const handleSendMessage = async (text: string) => {
        ...
        try {
            const messages = [...chatHistory, { role: 'user', content: text }].map(m => ({ role: m.role === 'ai' ? 'assistant' : 'user', content: m.content }));
    ...
    useEffect(() => {
        ...
        const targetName = metaCourse.trim().toLowerCase();
        const match = targetName ? coursesList.find((c: any) => c.name.toLowerCase() === targetName) : null;
    ...
    useEffect(() => {
        ...
        const savedKey = `lms_notes_teacher_state_${userId || 'guest'}`;
        ...
        if (savedRaw) {
            try {
                ...
                console.error('[Teacher LMS Notes] Failed to parse saved state:', e);
            }
        }
    ...
    // Persist state on topic change
    useEffect(() => {
        if (userId && selectedCourseId && selectedSubjectId && selectedTopicId) {
            localStorage.setItem(`lms_notes_teacher_state_${userId}`, JSON.stringify({
                            ...
                            <AnimatePresence>
                                {showProfileMenu && (
                                    <>
                                        ...
                                        >
                                            <button onClick={() => window.location.href = '/dashboard/teacher'} className="w-full flex items-center gap-3 px-5 py-3 text-sm font-bold text-slate-700 hover:bg-slate-50 hover:text-emerald-600 transition-colors">
                        ...
                        {showAIPanel && (
                            ...
                            >
                                    ...
                                    {chatHistory.length === 0 ? (
                                        <>
                                            <div className="text-center mb-4 mt-4">
                                                ...
                                                <p className="text-sm text-slate-600 font-medium">Hi {userName.split(' ')[0]}! Need help building material for <strong className="text-slate-800">{currentTopic?.name || 'this topic'}</strong>?</p>
                                            </div>
                                ...
                                <div className="p-4 border-t border-slate-100 bg-white flex-shrink-0">
                                    <div className="flex items-center justify-start mb-3 px-2">

...
export default function MentorDashboardClient() {
    ...
    const mentorTabConfig = [
        { id: 'mentees' as const, label: 'My Mentees', icon: Users, gradient: 'from-emerald-500 to-violet-600' },
    ...
    const renderMenteesTab = () => (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="md:col-span-1 bg-white border border-slate-200 rounded-3xl shadow-lg overflow-hidden">
                <div className="bg-gradient-to-b from-emerald-50/50 to-white p-6 border-b border-slate-100">
                    <h3 className="text-lg font-bold text-slate-800 flex items-center gap-2">
                        <Users className="w-5 h-5 text-emerald-600" /> My Mentees
                    </h3>
                </div>
                    ...
                    {currentMentees.map(mentee => (
                        <button key={mentee.id} onClick={() => setSelectedMenteeId(mentee.id)}
                            className={`w-full text-left p-4 rounded-xl border-2 transition-all flex items-center justify-between ${
                                selectedMenteeId === mentee.id ? 'border-emerald-400 bg-emerald-50 shadow-sm' : 'border-slate-200 hover:border-emerald-200 hover:bg-slate-50'
                            }`}>
                            <div>
                                <div className="font-bold text-slate-900">{mentee.studentName}</div>
                                <div className="text-sm text-slate-500">Year: {mentee.year}</div>
                            </div>
                            <ChevronRight className={`w-5 h-5 ${selectedMenteeId === mentee.id ? 'text-emerald-600' : 'text-slate-400'}`} />
                        </button>
                    ))}
                </div>
            </div>
            ...
            <div className="md:col-span-2 bg-white border border-slate-200 rounded-3xl shadow-lg overflow-hidden min-h-[400px]">
                {selectedMentee ? (
                    <div>
                        <div className="bg-gradient-to-b from-emerald-50/50 to-white p-6 border-b border-slate-100 flex justify-between items-center">
                            <div>
                                <h2 className="text-xl font-bold text-slate-900">{selectedMentee.studentName}</h2>
                                <p className="text-xs font-bold text-emerald-600 uppercase tracking-widest mt-1">Academic Year: {selectedMentee.year}</p>
                            </div>
                            <button onClick={() => setActiveTab('meetings')}
                                className="px-4 py-2.5 bg-gradient-to-r from-emerald-500 to-violet-600 text-white rounded-xl font-bold text-sm hover:shadow-lg hover:shadow-emerald-500/25 transition-all">
                                Schedule Meeting
                            </button>
                        </div>
                        ...
                        <div className="p-6 space-y-6">
                            ...
                            <div>
                                <h3 className="text-sm font-bold text-slate-800 mb-4 flex items-center gap-2 uppercase tracking-widest">
                                    <BarChart className="w-4 h-4 text-emerald-600" /> Progress Overview
                                </h3>
                                <div className="h-48 flex items-end justify-between px-4 pb-4 border-b border-l border-slate-200 bg-slate-50/50 rounded-bl-xl pt-8">
                                    {selectedMentee.progressData.map((val, idx) => (
                                        <div key={idx} className="flex flex-col items-center w-full">
                                            <div className="w-10 bg-gradient-to-t from-emerald-500 to-violet-400 rounded-t-lg transition-all duration-1000 ease-out hover:from-emerald-600 hover:to-violet-500 cursor-pointer relative group flex items-end justify-center pb-2 text-white text-xs font-bold"
                                                style={{ height: `${val}%` }}>
                                                {val}%
                                            </div>
        ...
        <div className="space-y-6">
            {!selectedMentee ? (
                <div className="p-12 text-center bg-white border border-slate-200 rounded-3xl shadow-lg">
                    ...
                    <button onClick={() => setActiveTab('mentees')} className="px-6 py-2.5 bg-gradient-to-r from-emerald-500 to-violet-600 text-white font-bold rounded-xl hover:shadow-lg transition-all">
                        Go to Mentees
                    </button>
                </div>
            ) : (
                    ...
                    <div className="bg-white border border-slate-200 rounded-3xl shadow-lg overflow-hidden">
                        <div className="bg-gradient-to-b from-emerald-50/50 to-white p-6 border-b border-slate-100">
                            <div className="flex items-center justify-between">
                                ...
                                <span className="text-xs font-bold px-3 py-1.5 bg-emerald-100 text-emerald-700 rounded-xl">{selectedMentee.studentName}</span>
                            </div>
                        </div>
                                        ...
                                        <div className="flex justify-between items-start mb-3">
                                            ...
                                            <div className="flex items-center gap-2">
                                                {meeting.isSigned ? (
                                                    ...
                                                    <button onClick={() => signMeeting(meeting.id)} className="text-xs font-bold text-emerald-600 bg-emerald-100 hover:bg-emerald-200 px-3 py-1.5 rounded-xl transition flex items-center gap-1.5">
            ...
            <div className="w-1/3 border-r border-slate-200 bg-gradient-to-b from-slate-50 to-white flex flex-col">
                <div className="p-4 border-b border-slate-200">
                    ...
                    <div className="relative">
                        ...
                        <input type="text" placeholder="Search..." className="w-full pl-9 pr-3 py-2.5 text-sm border-slate-200 rounded-xl outline-none focus:border-emerald-500 border bg-white font-medium" />
                    </div>
                </div>
                <div className="overflow-y-auto flex-1 p-3 space-y-1.5">
                    <button onClick={() => setSelectedMenteeId(null)}
                        className={`w-full text-left p-3 rounded-xl flex items-center gap-3 transition-all ${!selectedMenteeId ? 'bg-gradient-to-r from-emerald-500 to-violet-600 text-white shadow-md' : 'hover:bg-slate-50 text-slate-700'}`}>
                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${!selectedMenteeId ? 'bg-white/20' : 'bg-emerald-100 text-emerald-700'}`}><Users className="w-5 h-5"/></div>
                        <div>
                    ...
                    {currentMentees.map(mentee => (
                        <button key={mentee.id} onClick={() => setSelectedMenteeId(mentee.id)}
                            className={`w-full text-left p-3 rounded-xl flex items-center gap-3 transition-all ${selectedMenteeId === mentee.id ? 'bg-gradient-to-r from-emerald-500 to-violet-600 text-white shadow-md' : 'hover:bg-slate-50 text-slate-700'}`}>
                        ...
                        .map(chat => {
                            ...
                            return (
                                <div key={chat.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                                    <div className={`max-w-[70%] rounded-2xl p-4 ${isMe ? 'bg-gradient-to-br from-emerald-600 to-violet-700 text-white rounded-tr-sm shadow-md shadow-emerald-500/15' : 'bg-white border border-slate-200 text-slate-800 rounded-tl-sm shadow-sm'}`}>
                                        {!isMe && <div className="text-[11px] font-bold text-emerald-600 mb-1 uppercase tracking-widest">{chat.senderName}</div>}
                                        <div className="text-sm leading-relaxed">{chat.message}</div>
                                        <div className={`text-[10px] mt-1.5 text-right ${isMe ? 'text-emerald-200' : 'text-slate-400'} font-bold`}>
                ...
                <div className="p-4 bg-gradient-to-t from-slate-50 to-white border-t border-slate-100">
                    <div className="flex flex-wrap gap-3">
                        <input type="text" placeholder={!selectedMenteeId ? "Type a broadcast message..." : "Type a message..."}
                            className="flex-1 border border-slate-200 rounded-xl px-4 py-3 outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 text-sm font-medium transition-all"
                        ...
                        />
                        <button className="w-12 h-12 bg-gradient-to-br from-emerald-500 to-violet-600 text-white rounded-xl flex items-center justify-center hover:shadow-lg hover:shadow-emerald-500/25 transition-all shrink-0 hover:scale-105 active:scale-95"
                            onClick={() => {
                    ...
                    <div className="relative flex items-center justify-between">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-emerald-300/80 text-sm font-medium">Monitor progress, schedule meetings, and provide assessments</p>
                            </div>
                        </div>
                        {isCoordinator && (
                            <div className="hidden sm:flex items-center gap-1 bg-white/10 backdrop-blur-sm border border-white/10 p-1 rounded-xl">
                                <button onClick={() => setViewMode('mentor')}
                                    className={`px-4 py-2 font-bold text-sm rounded-lg transition-all ${viewMode === 'mentor' ? 'bg-white text-emerald-700 shadow-sm' : 'text-white/70 hover:text-white'}`}>

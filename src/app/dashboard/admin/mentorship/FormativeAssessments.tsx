...
export default function FormativeAssessments({ students }: Props) {
    ...
    const generatePDF = async (): Promise<jsPDF | null> => {
        ...
        try {
            ...
            if (logoUrl) {
                try {
                    // This assumes base64 or a reachable URL. In a robust setup, it might need to be resolved.
                    // For now, we simulate the logo text if we can't easily draw an external URL, 
                    // or we'd addImage if it's a known format. We'll fallback to a text representation if addImage isn't available.
            ...
            // 3. Mentorship Program (Changed to context-specific name)
            pdf.setFontSize(14);
            pdf.text(activeAssessment.name, pageWidth / 2, y, { align: 'center' });
            y += 8;

            // 4. Assessment Title (Subtitle)
            pdf.setFontSize(12);
            pdf.setFont("helvetica", "normal");
            pdf.text('Formative Assessment', pageWidth / 2, y, { align: 'center' });
            y += 15;
    ...
    const handleSharePDF = async () => {
        ...
        try {
            ...
            const file = new File([blob], `${activeAssessment?.name.replace(/\s+/g, '_')}_Formative.pdf`, { type: 'application/pdf' });
            
            if (navigator.canShare && navigator.canShare({ files: [file] })) {
                await navigator.share({
                    files: [file],
                    title: 'Formative Assessment Records',
                    text: `Here are the uploaded records for ${activeAssessment?.name}.`
                });
            } else {
                pdf.save(`${activeAssessment?.name.replace(/\s+/g, '_')}_Formative.pdf`);
            }
        } catch (error) {
                    ...
                    <table className="w-full text-left">
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {students.map(student => {
                                // Find their record, if it exists
            ...
            <div className="p-6 max-w-2xl mx-auto h-full overflow-y-auto">
                <div className="mb-8">
                    <button onClick={() => setView('list')} className="text-sm font-semibold text-slate-500 hover:text-slate-800 mb-4 block">
                        &larr; Back to Formative Assessments
                    </button>
                    <h3 className="text-2xl font-bold text-slate-900">Create Formative Assessment</h3>
                    ...
                    <div className="space-y-4 bg-slate-50 p-5 border border-slate-200 rounded-xl">
                        <div>
                            ...
                            <input 
                                ...
                                placeholder="e.g. 1st Formative Assessment"
        ...
        <div className="p-6 h-full flex flex-col">
            <div className="flex justify-between items-center mb-6">
                <div>
                    ...
                    <p className="text-slate-500 text-sm mt-1">Manage and track continuous student assessments.</p>
                </div>
                <button 
            ...
            {formativeAssessments.length === 0 ? (
                <div className="flex-1 flex flex-col items-center justify-center border-2 border-dashed border-slate-200 rounded-2xl bg-slate-50 text-slate-400 p-8">
                    ...
                    <p className="font-semibold text-lg text-slate-600 mb-1">No Formative Assessments found.</p>
